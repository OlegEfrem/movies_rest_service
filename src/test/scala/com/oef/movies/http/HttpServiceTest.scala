...
class HttpServiceTest extends ApiSpec with Protocol {
  ...
  "registration" should {
    ...
    "register a new movie" in {
      Put(generalUrl(), requestEntity) ~> httpService.routes ~> check {
    ...
    "reject registering an existing movie" in {
      ...
      Put(generalUrl(imdbId), requestEntity) ~> httpService.routes ~> check {
    ...
    "fail validation if path and body resource identifiers differ" in {
      Put(generalUrl("differentImdb"), requestEntity) ~> httpService.routes ~> check {
  ...
  "reservation" should {
    ...
    "reserve an existing movie" in {
      Patch(generalUrl(), requestEntity) ~> httpService.routes ~> check {
        response.status shouldBe OK
    ...
    s"return HTTP-$Conflict if no seats left" in {
      ...
      Patch(generalUrl(imdbId), requestEntity) ~> httpService.routes ~> check {
        response.status shouldBe Conflict
    ...
    "fail validation if path and body resource identifiers differ" in {
      Patch(generalUrl("differentImdb"), requestEntity) ~> httpService.routes ~> check {
        response.status shouldBe Forbidden
        responseAs[String] shouldBe
          "validation failed: resource identifiers from the path [imdbId=differentImdb, screenId=screen_123456] " +
          "and the body: [imdbId=tt0111161, screenId=screen_123456] do not match"
  ...
  "retrieval" should {
    ...
    "return an existing movie's info" in {
      Get(generalUrl()) ~> httpService.routes ~> check {
        response.status shouldBe OK
    ...
    s"return HTTP-$NotFound for non existing movie/screen combination" in {
      Get(generalUrl(imdbId = "NonExistingImdbId")) ~> httpService.routes ~> check {
        status shouldBe NotFound
